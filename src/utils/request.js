...
// 创建默认配置
const defaultConfig = {
  baseURL: '/api', // 使用相对路径，通过代理转发到http://localhost:8080
...
function myAxios(axiosConfig, customOptions, loadingOptions) {
  ...
  // 自定义配置
  let custom_options = Object.assign({}, defaultOptions, customOptions);
    ...
    config => {
      ...
      if (custom_options.loading) {
        ...
        if(LoadingInstance._count === 1) {
          LoadingInstance._target = ElLoading.service(loadingOptions || defaultLoadingOptions);
        }
      }
      // 自动携带token
